...
Changing function #3 worked with wang YiXuan and Tyler law
...
combain(
	...
	List.fold_right (fun x (a,b) ->if is_elem_by f x ls then
	([],a::b)
	else 
	(x::a,b))
	ele ([],[])
...
Changing the function#1 worked with  Wang Yixuan and Tyler Law
...
(List.filter del (split_by (=) x ls2))::y)
ls []
...
let helper ls =
let ele1 =if (take 1 ls = take 1 (drop 1 ls)) then [] else [(1,2)] in
...
let ele56 = 
if
List.sort compare(dedup(List.concat(List.concat((take 1 ls) ::(take 1 (drop 1 ls)):: (take 1 (drop 2 ls))::[])))) =
List.sort compare (dedup(List.concat(List.concat((take 1 (drop 4 ls)) :: (take 1 (drop 5 ls)) :: [] )))) 
then []
else [(5,6)] in
let ele1112 =
if 
List .sort compare((List.concat(List.concat(take 1 (drop 6 ls) ::(take 1 (drop 9 ls)) ::[] )))) =
List.sort compare ( (List.concat(List.concat(take 1 (drop 10 ls) :: (take 1 (drop 11 ls)) ::[]))))
then []
else [(11,
12)] in
let ele1718 = 
if List.sort compare((List.concat(List.concat(take 1 (drop 13 ls) :: (take 1 (drop 15 ls)) ::[])))) =
List.sort compare (( List.concat(List.concat(take 1 (drop 16 ls):: (take 1 (drop 17 ls)) ::[]))))
then []
else [(17,18)] in
...
let checksa ls =
let
front = List.sort compare (dedup(List.concat(List.concat( (take 1 ls) 
:: (take 1 (drop 2 ls)) :: (take 1 (drop 6 ls)) ::(take 1 (drop 8 ls)) 
:: (take 1 (drop 12 ls)) :: (take 1 (drop 14 ls)) :: []))))
in let 
back = List.sort compare (dedup(List.concat(List.concat((take 1 
(drop 18 ls)) :: (take 1 (drop 19 ls)) :: (take 1 (drop 20 ls)) ::
(take 1 (drop 21 ls )) :: (take 1 (drop 22 ls)) :: (take 1 (drop 23 ls))::[]))))
in
front = back
...
Changing the function #2 with Wang YiXuan and Tyler Law
