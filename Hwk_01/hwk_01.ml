...
let square_approx n accu = 
	...
	let guess = 0.5 *.(lower+.upper) in
	if (upper-.lower) <= accu then (lower, upper) else
...
let rec max_list ls =
	...
	| x1::[] ->x1
	| x1::x2::[] ->bigger x1 x2
	| x1::x2::rest ->bigger x1 (max_list (x2::rest))
...
(* hw question 7*)
let rec drop num ls =
	if num<0 then raise (Failure "Positive number please :) ") else
	let count = 0 in
	if count = num then ls
	else
	match ls with 
	|[] ->[]
...
let rec helper11 ls incr=
	match ls with
	|[] ->[]
	|x1::rest ->( (x1+incr)::(helper11 rest	incr))
